# 🚀 GitHub Actions 自动部署指南

## 概述

本项目已配置 GitHub Actions 自动化部署工作流。当您将代码推送到 `main` 分支时，系统将自动：

1. **同步代码** 到火山云服务器
2. **构建 Docker 镜像**
3. **重启服务** 完成部署

## 🏗️ 架构说明

```
┌─────────────────────────────────────────────────────────────────┐
│                         GitHub                                  │
│  ┌─────────────┐    push     ┌──────────────────┐              │
│  │   本地代码   │ ─────────→ │   GitHub Repo    │              │
│  └─────────────┘             └────────┬─────────┘              │
└───────────────────────────────────────│─────────────────────────┘
                                        │ trigger
                                        ▼
                              ┌──────────────────┐
                              │  GitHub Actions  │
                              └────────┬─────────┘
                                       │ SSH + rsync
                     ┌─────────────────┴─────────────────┐
                     ▼                                   ▼
           ┌─────────────────┐                 ┌─────────────────┐
           │   Server A      │                 │   Server B      │
           │  (主服务器)      │                 │  (Worker节点)   │
           │ 115.191.30.209  │                 │ 115.190.185.29  │
           └─────────────────┘                 └─────────────────┘
```

## 📋 配置步骤

### 步骤 1: 生成并配置 SSH 密钥

运行设置脚本，生成专用于 GitHub Actions 的 SSH 密钥：

```bash
cd /Users/lingtengjiao/AI项目/amz_comment_translate
chmod +x deploy/setup-github-ssh.sh
./deploy/setup-github-ssh.sh
```

脚本会：
- 生成新的 Ed25519 SSH 密钥对
- 自动将公钥添加到两台服务器
- 输出私钥内容供您复制

### 步骤 2: 配置 GitHub Secrets

1. 访问仓库设置页面：
   https://github.com/lingtengjiao/amz_comment_translate/settings/secrets/actions

2. 点击 **"New repository secret"** 添加以下 Secrets：

| Secret 名称 | 值 |
|------------|---|
| `SSH_PRIVATE_KEY` | 脚本输出的私钥内容（完整复制，包括 `-----BEGIN` 和 `-----END` 行） |
| `SERVER_A_IP` | `115.191.30.209` |
| `SERVER_B_IP` | `115.190.185.29` |
| `SERVER_USER` | `root` |
| `QWEN_API_KEY` | `sk-bb9ae189dc3b4b85a9d5bd156254de76` |

### 步骤 3: 验证配置

1. 推送代码到 main 分支：
   ```bash
   git add .
   git commit -m "feat: 添加 GitHub Actions 自动部署"
   git push origin main
   ```

2. 查看 Actions 执行状态：
   https://github.com/lingtengjiao/amz_comment_translate/actions

## 🔄 工作流说明

### 自动触发

- **Push 到 main 分支**：自动触发部署
- **排除**：仅修改 `.md` 文件或 `docs/` 目录不触发

### 手动触发

1. 访问 Actions 页面
2. 选择 **"🚀 Deploy to Production"** 工作流
3. 点击 **"Run workflow"**
4. 可选择部署目标：
   - `all`: 部署到所有服务器（默认）
   - `server-a`: 仅部署主服务器
   - `server-b`: 仅部署 Worker 节点

## 📊 部署流程详解

### Server A (主服务器) 部署

1. **代码同步**: 通过 rsync 增量同步，排除非必要文件
2. **环境配置**: 自动生成 `.env` 文件
3. **镜像构建**: `docker-compose -f docker-compose-master.yml build`
4. **优雅重启**: 
   - 先重启后端和 Worker（保持数据库运行）
   - 再重启前端和 Nginx
5. **健康检查**: 验证 API 和前端可访问

### Server B (Worker 节点) 部署

1. **代码同步**: 仅同步 backend 相关代码
2. **环境配置**: 配置 `MASTER_IP` 指向 Server A
3. **镜像构建**: `docker-compose -f docker-compose-worker.yml build`
4. **重启 Worker**: 重建所有 Worker 容器
5. **验证注册**: 检查 Worker 是否在 Flower 中注册

## ⚠️ 注意事项

### 密钥安全

- **永远不要** 将 SSH 私钥提交到代码仓库
- 私钥仅存储在 GitHub Secrets 中
- 如果私钥泄露，立即在服务器上删除对应公钥并重新生成

### 部署期间

- 部署过程中服务会短暂中断（约 1-2 分钟）
- 数据库和 Redis 不会重启，数据安全
- 如果部署失败，服务会自动回滚到之前的状态

### 回滚

如需回滚到之前版本：

```bash
# 方法 1: Git 回滚
git revert HEAD
git push origin main

# 方法 2: 手动回滚
# SSH 到服务器执行
docker-compose -f docker-compose-master.yml logs --tail 100
# 检查问题后手动恢复
```

## 🔍 故障排查

### Actions 执行失败

1. 查看 Actions 日志获取详细错误
2. 常见问题：
   - SSH 连接失败 → 检查 SSH_PRIVATE_KEY 是否正确
   - rsync 失败 → 检查服务器磁盘空间
   - Docker 构建失败 → 检查 Dockerfile 语法

### 部署后服务异常

```bash
# SSH 到对应服务器
ssh root@115.191.30.209

# 查看容器状态
docker-compose -f docker-compose-master.yml ps

# 查看日志
docker-compose -f docker-compose-master.yml logs -f app-backend
```

## 📁 相关文件

| 文件 | 说明 |
|------|------|
| `.github/workflows/deploy.yml` | GitHub Actions 工作流配置 |
| `deploy/setup-github-ssh.sh` | SSH 密钥配置脚本 |
| `docker-compose-master.yml` | 主服务器 Docker 配置 |
| `docker-compose-worker.yml` | Worker 节点 Docker 配置 |

## 🔗 快速链接

- **GitHub Actions**: https://github.com/lingtengjiao/amz_comment_translate/actions
- **GitHub Secrets**: https://github.com/lingtengjiao/amz_comment_translate/settings/secrets/actions
- **前端**: http://115.191.30.209:3000
- **API**: http://115.191.30.209:8000
- **Flower**: http://115.191.30.209:5555
