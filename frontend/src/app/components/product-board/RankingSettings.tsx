...
export type RankingMetric = 'major' | 'minor';
...
interface RankingSettingsProps {
  ...
  rankingRanges: RankingRange[];
  onSave: (ranges: RankingRange[], metric: RankingMetric) => void;
  onClose: () => void;
}
...
export function RankingSettings({ isOpen, rankingMetric, rankingRanges, onSave, onClose }: RankingSettingsProps) {
  const [ranges, setRanges] = useState<RankingRange[]>([...rankingRanges]);
  const [metric, setMetric] = useState<RankingMetric>(rankingMetric);

  // 当 props 变化时，同步到内部状态
  useEffect(() => {
    setRanges([...rankingRanges]);
    setMetric(rankingMetric);
  }, [rankingRanges, rankingMetric]);

  if (!isOpen) return null;

  const handleAddRange = () => {
    const newRange: RankingRange = {
      id: `rank-${Date.now()}`,
      name: '新排名区间',
      min: 1,
      max: 10,
  ...
  const handleUpdateRange = (id: string, updates: Partial<RankingRange>) => {
  ...
  const handleSave = () => {
    ...
    (async () => {
      try {
        await onSave(sortedRanges, metric);
      } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-[600px] max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900">自定义排名设置</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {/* 排名指标选择 */}
          <div className="mb-6">
            <label className="text-sm font-medium text-gray-700 mb-2 block">排名指标</label>
            <div className="flex gap-2">
              {(Object.keys(metricLabels) as RankingMetric[]).map((m) => (
                <button
                  ...
                  style={metric === m ? { backgroundColor: '#FF1B82' } : {}}
                >
                  {metricLabels[m]}
                </button>
              ))}
                  ...
                  <div className="flex-1 space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="例如：Top 1-10"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">排名起始</label>
                        <input
                          type="text"
                          value={range.min}
                          onChange={(e) => {
                            ...
                            if (value === '' || /^\d+$/.test(value)) {
                              handleUpdateRange(range.id, { min: value === '' ? 1 : Number(value) });
                            }
                          }}
                          className="w-full border border-gray-200 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 text-sm"
                          placeholder="1"
                      ...
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">排名结束</label>
          ...
          >
            ...
            <span className="text-sm font-medium">添加排名区间</span>
          </button>
        </div>
