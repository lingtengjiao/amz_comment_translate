...
interface SalesRangeSettingsProps {
  isOpen: boolean;
  salesRanges: SalesRange[];
  onSave: (ranges: SalesRange[]) => void;
  onClose: () => void;
}

export function SalesRangeSettings({ isOpen, salesRanges, onSave, onClose }: SalesRangeSettingsProps) {
  const [ranges, setRanges] = useState<SalesRange[]>([...salesRanges]);

  // 当 salesRanges prop 变化时，同步到内部状态
  useEffect(() => {
    setRanges([...salesRanges]);
  }, [salesRanges]);
  ...
  const handleUpdateRange = (id: string, updates: Partial<SalesRange>) => {
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-[600px] max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900">自定义销量区间</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6">
                  ...
                  <div className="flex-1 space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="例如：销量0-1000"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">最小销量</label>
                        <input
                          type="text"
                          value={range.min}
                          onChange={(e) => {
                      ...
                      <div>
                        <label className="text-xs text-gray-500 mb-1 block">最大销量</label>
          ...
          >
            ...
            <span className="text-sm font-medium">添加销量区间</span>
          </button>
        </div>
