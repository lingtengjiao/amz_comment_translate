...
| 9 | 翻译调度 | `task_check_pending_translations` | 定时(15秒) | 检查并补发翻译任务 |
| 10 | 队列消费 | `task_process_ingestion_queue` | 定时(5秒) | 从Redis队列批量入库 |
...
6. **聚合**: 统计分析 + 数据可视化
7. **生成**: 4种角色化报告 + N-Way对比分析
8. **展示**: React前端 + 双语阅读 + 图表展示
